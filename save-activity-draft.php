...
$activity_id = $activity_title = $mag_id = $activity_type_id = $activity_published_date = $activity_status = $article_id = $activity_path = $image_path = $activity_content = $activity_style = $audio_path = $quick_tips = $activity_html = $activity_result = $theme = $topic_words = $fiction = $difficulty_level = $word_count = $author = '';

$stmt = $mysqli->prepare("SELECT  `activity_id`, `activity_title`, `mag_id`, `activity_type_id`, `activity_published_date`, `activity_status`, `article_id`, `activity_path`, `image_path`, `activity_content`, `activity_style`, `audio_path`, `quick_tips`, `activity_html`, `activity_result`, `theme`, `topic_words`, `fiction`, `difficulty_level`, `word_count`, `author`,`issue_no`,`audio_support`,`act_year` from edu_activity_dummy where activity_id =?");

$stmt->bind_param("s", $param_user_id);
// Set parameters 
$param_user_id = $_REQUEST['actID'];
$stmt->execute();
$stmt->bind_result($activity_id, $activity_title, $mag_id, $activity_type_id, $activity_published_date, $activity_status, $article_id, $activity_path, $image_path, $activity_content, $activity_style, $audio_path, $quick_tips, $activity_html, $activity_result, $theme, $topic_words, $fiction, $difficulty_level, $word_count, $author, $issue_no, $audio_support, $act_year);
$stmt->fetch();
$stmt->close();
$mag_type_id2 = '';
    ...
    <div id="page-wrapper">
      ...
      <div class="container-fluid">
        <div class="row"><br>
          ...
          <div class="col-lg-12">
            <h3 id="grid-responsive-resets" class="headertext">Save Activity Draft</h3>
            <br>
            ...
            <form role="form" action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]); ?>" method="post" enctype="multipart/form-data" id="Myform">
              <fieldset>
                      ...
                      <span>
                        <select name="activity_type_id" id="activity_type_id" class="form-control formfield selectheadback">
                          ...
                          if ($stmt = $mysqli->prepare("SELECT activity_type_id,activity_type FROM edu_activity_type where activity_type_status=?")) {
                            ...
                            $stmt->bind_result($act_type_id1, $act_type);
                            $sr = 1;
                            /* fetch values */
                            echo " <option style='font-family:Arial, Helvetica, sans-serif !important;'>Select Type of Activity</option>";
                            while ($stmt->fetch()) {
                              echo " <option style='font-family:Arial, Helvetica, sans-serif !important;'  value='" . $act_type_id1 . "' " . (($act_type_id1 == $activity_type_id) ? 'selected="selected"' : "") . ">" . $act_type . "</option>";
                            }
                          }
                          ?>
                        </select>
                  ...
                  <div class="col-lg-4">
                    ...
                    <div class="form-group">
                      <span id='dataArtNo'>
                        ...
                        $stmt->execute();
                        $stmt->bind_result($article_title_disp);
                        $stmt->fetch();
                        $stmt->close();
                        if($article_id==0){$article_title_disp='Nil';}
                        ...
                        $sr = 1;
                        //echo "<input list='texttype' id='_type' name='article_id' class='form-control formfield' required value='" . $article_title_disp . "' /></label>";
                        echo "<select id='article_id' name='article_id' class='form-control formfield'>";
                        while ($stmt->fetch()) {
                          echo "<option style='font-family:Arial, Helvetica, sans-serif !important;' value='" . $essay_type . "' " . (($essay_type_id == $article_id) ? 'selected="selected"' : "") . ">" . $essay_type . "</option>";
                          $sr++;
                        }
                        echo "</select>";*/ ?>
                       <input id='article_id' name='article_id' class="form-control formfield" type="text" value="<?php echo $article_title_disp;?>">
                      </span>
                    </div>
                  </div>
                  <div class="col-lg-4">
                    ...
                    <div <?php if ($_REQUEST['actID'] == 0) { ?> class="audioshow" <?php } ?> id="audioshow">
                      ...
                      <div class="form-group">
                        <span>
                          ...
                          $stmt = $mysqli->prepare("SELECT path FROM edu_activity_audio_dummy where activity_id=?");
                          /* Bind parameters */
                ...
                <input type="hidden" name="activity_html" id="activity_html" value="<?php 
                    ...
                    echo $act_html;
                ?>">
                  <input type="hidden" name="activity_result" id="activity_result" value="<?php echo htmlspecialchars($activity_result); ?>">
                </span>
                ...
                <div class="row">
                  ...
                  <div class="col-lg-12 activity" id='unscramble'>
                    ...
                    <div class="form-group">
                      ...
                      <div class="row container col-lg-11" id='scramble_block'>
                  ...
                  <div class="col-lg-4" style="margin-top:20px;">
                    ...
                    <div class="form-group">
                      <span>
                        <input type="submit" class="btn btn-lg btn-success btn-block formfield" value="Update Draft" id="savedraft">
  ...
  <script>
    $(document).ready(function() {
      ...
      $('#savedraft').on('click', function(e) {
        //do button 1 thing
        if ($("#activity_type_id").val() == "8") {
          var actURL = 'data/editTodarftactivityscrambled.php?actId=<?php echo $_REQUEST['actID']; ?>';
        } else if ($("#activity_type_id").val() == "7") {
          var actURL = 'data/editTodarftactivitysituationalwrite.php?actId=<?php echo $_REQUEST['actID']; ?>';
        }
        ...
        $('#Myform ').on('submit', function(e) {
          ...
          $.ajax({
            type: 'POST',
            url: 'data/editTodarftactivity.php?actId=<?php echo $_REQUEST['actID']; ?>',
            data: data,
            cache: false,
            processData: false,
            contentType: false,
            ...
            success: function(data) { //console.log(response);
              ...
              setTimeout(function() {
                 window.location = 'activity-draft.php?actID=<?php echo $_REQUEST['actID']; ?>';
              }, 2000);
            }
          });
    ...
    $(document).ready(function() {
      ...
      if ($("#activity_type_id").val() == "8") {
        ...
        for (var i = 1; i <= span_value; i++) {
          ...
          if (val != "" && val1 != "") {
            ...
            activity_html += `<div class="row"><div class="col-md-4 align-middle">Scrambled: ` + val + `  &nbsp &nbsp </div><div class="col-md-3"><input class="inp" name="sc` + i + `" id="sc` + i + `" data-answer="` + val1 + `"> <br></div></div> `;
      ...
      if ($("#activity_type_id").val() == "3") {
        ...
        $("#activity_result").val(JSON.stringify(arr0));
          ...
          if ($(this).hasClass("scrambleinput")) {
            ...
            for (var i = 1; i <= span_value; i++) {
              ...
              if (val != "" && val1 != "") {
                ...
                $("#disp_scram").append(`Scrambled: ` + val + ` &nbsp &nbsp <input class="formfield inp" name="sc` + i + `" id="sc` + i + `" data-answer="` + val1 + `"> `);
                activity_html += `
                <div class="row"><div class="col-md-4 align-middle">Scrambled: ` + val + `  &nbsp &nbsp </div><div class="col-md-3"><input class="inp" name="sc` + i + `" id="sc` + i + `" data-answer="` + val1 + `"> <br></div></div>
                `;
              }
            }
