...
if ($stmt = $mysqli->prepare("SELECT a.audio_path from edu_activity a inner join edu_school_subscription b on a.activity_id=b.activity_id  where a.activity_status=? and b.school_subscription_status=? and b.school_id= ? and a.activity_id=? and a.mag_id= ? and a.activity_id= ?")) {
...
*/
			...
			//-------------------------situational writing activity--------------------------------
			/*$(document).on("submit", "#activitySWsubmit", function(e) { // onclic function
				e.preventDefault();

				var act_id = <?php echo $_REQUEST['actID']; ?>;
				var art_id = <?php echo $_REQUEST['artID']; ?>;
				var mag_id = <?php echo $_REQUEST['magID']; ?>;
				var actSWperformed = 1;
				var situationalWrite = $("#situationalWrite").val();
				$.ajax({
					type: 'POST',
					url: 'data/saveActivityperformed.php',
					data: {
						...
						art_id: art_id,
						mag_id: mag_id,
						actSWperformed: actSWperformed,
						situationalWrite: situationalWrite
					},
					cache: false,
					success: function(data) {
						$('#successAll').modal({
    ...
    $('#spanHigh').removeClass('highlight');
		...
		$(document).ready(function() {
			...
			$(document).on("click", "#showmenu", function(e) {
			 ...
			 //$('#situationalwriteId1').on('submit', function (e) { alert(1);
			 $(document).on("submit", "#situationalwriteId", function(e) { 
					       e.preventDefault();
					       var data = new FormData(this);
						   data.append('activityId', '<?php echo $_REQUEST['actID']; ?>');
						   data.append('articleId', '<?php echo $_REQUEST['artID']; ?>');
						   data.append('magId', '<?php echo $_REQUEST['magID']; ?>');
						   data.append('marks', 30);
							
					       $.ajax({
									type: 'POST',
								    url: 'data/saveActivityanswered.php',
									data: data,
									cache: false,
									processData:false,
									contentType: false,
