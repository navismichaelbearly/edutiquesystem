...
{ 
	...
	{ 
	   $stmt = $mysqli->prepare("Select mag_type_id from edu_mag_type where mag_type=?");
		...
		{
		  //echo 1;
		  //echo "<script>alert('Issue No. exists in the system') <script>"; 
		}else{  
		      $varMagid = $mag_type_idnew;
			   ...
			   $uploadStatus = 1; 
					...
					if($uploadStatus == 1){ 
						...
						$stmt = $mysqli->prepare("INSERT into edu_magazine (mag_title,mag_issue,mag_published_date, mag_status, mag_type_id, mag_image_path,tc_image,editors_note) 
								values(?,?,?,?,?,?,?,?)");
						$stmt->bind_param("ssssssss", $param_mag_title, $param_mag_issue, $param_mag_published_date, $param_mag_status, $param_mag_type_id, $param_mag_image_path, $param_tc_image, $param_editors_note);
						$param_mag_title = $mag_theme;
						$param_mag_issue= $issue_no;
						$param_mag_published_date =$newDate;
						$param_mag_status =$active;
						$param_mag_type_id = $varMagid;
						$param_mag_image_path=$uploadedFile;
						$param_tc_image = $uploadedFiletc;
