...
if (isset($_POST['author']) || isset($_POST['word_count']) || isset($_POST['file']) || isset($_POST['description']) || isset($_POST['difficulty_level']) || isset($_POST['fiction']) || isset($_POST['audio_support']) || isset($_POST['topic_words']) || isset($_POST['text_type']) || isset($_POST['genre']) || isset($_POST['theme']) || isset($_POST['issue_no']) || isset($_POST['art_year']) || isset($_POST['content']) || isset($_POST['title']) || isset($_POST['mag_type'])) {
	...
	if (!empty($title)) {
		...
		if ($uploadStatus == 1) {
			...
			$stmt = $mysqli->prepare("UPDATE `edu_activity` SET `activity_title`=?,`mag_id`=?,`activity_type_id`=?,`article_id`=?,`image_path`=?,`activity_content`=?,`quick_tips`=?,`theme`=?,`topic_words`=?,`fiction`=?,`difficulty_level`=?,`word_count`=?,`author`=?,`issue_no`=?,`audio_support`=?, `activity_html`=?,`activity_result`=?,`act_year`=? WHERE activity_id=? ");
			$stmt->bind_param("sssssssssssssssssss", $param_article_title, $param_mag_id,  $param_activity_type_id,  $param_essay_type_id, $param_article_image, $param_article_content, $param_quick_tips, $param_theme, $param_topic_words, $param_fiction, $param_difficulty_level, $param_word_count, $param_author, $param_issue_no, $param_audio_support, $activity_html, $activity_result,$act_year, $param_aID);
			...
			$param_quick_tips = $_POST['quick_tips'];
