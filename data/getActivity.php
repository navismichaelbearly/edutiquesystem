...
if($_POST['actDetail'] != '')
{
  if ($stmt = $mysqli->prepare("SELECT a.activity_content, a.activity_html from edu_activity a inner join edu_school_subscription b on a.activity_id=b.activity_id  where a.activity_status=? and b.school_subscription_status=? and b.school_id= ? and a.article_id=? and a.mag_id=? and a.activity_id=?")) {
...
{
  if ($stmt = $mysqli->prepare("SELECT a.activity_style from edu_activity a inner join edu_school_subscription b on a.activity_id=b.activity_id  where a.activity_status=? and b.school_subscription_status=? and b.school_id= ? and a.article_id=? and a.mag_id=? and a.activity_id=?")) {
