...
if ($_POST['mag'] != '') {
	if ($stmt = $mysqli->prepare("SELECT a.mag_id,a.mag_title, a.mag_issue,a.mag_published_date,a.mag_image_path, b.mag_type from edu_magazine a inner join edu_mag_type b on a.mag_type_id = b.mag_type_id where a.mag_status=? and b.mag_type_status= ? " . $magVar)) {
		...
		foreach ($push_arr as $key => $value) {
			...
			if ($mag_id != null) {
				...
				echo "<td style='vertical-align:middle'><span><a href='edit-magazine.php?magID=".$mag_id."' style='text-decoration:none' ><i class='material-icons-outlined md-16 annocom' id='editArt' data-id='" . $mag_id . "'>create</i></a></td>";
				echo "</tr>" ;
				$sr++;
			}
		}
