...
if($_POST['button_id'] != '')
{
   if ($stmt = $mysqli->prepare("SELECT a.quick_tips from edu_activity a inner join edu_school_subscription b on a.activity_id=b.activity_id  where a.activity_status=? and b.school_subscription_status=? and b.school_id= ? and a.article_id=? and a.mag_id=? and a.activity_id=?")) {
	 ...
	 $stmt->bind_result($quick_tips);
	 $stmt->fetch();
	 if($quick_tips !=""){
	    echo "<button type='button' id='quickTips' class='btn btn-default btn-xl' style='background-color:#ffcc00; color:#ffffff; border:1px solid #d7ae09'>View Quick Tips!</button>";
	 }
     $stmt->close();
