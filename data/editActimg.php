...
if (isset($_POST['author']) || isset($_POST['file']) || isset($_POST['description']) || isset($_POST['difficulty_level']) ||  isset($_POST['audio_support']) ||  isset($_POST['theme']) || isset($_POST['issue_no']) || isset($_POST['art_year']) || isset($_POST['content']) || isset($_POST['title']) || isset($_POST['mag_type'])) {
	// Get the submitted form data 

	$activity_id = $_POST['dummyActid'];// main act id to be changed
	$author = $_POST['author'];
	...
	if (!empty($title)) {
		...
		/* Set parameters */
		$param_art_title = $_POST['article_id'];
		$stmt->execute();
		$stmt->bind_result($article_id_new);
		$stmt->fetch();
		$stmt->close();
        if($_POST['article_id']=='Nil'){$article_id_new=0;}
		if ($mag_id == '') {
		...
		if ($uploadStatus == 1) {
			...
			$stmt = $mysqli->prepare("UPDATE `edu_activity` SET `activity_title`=?,`mag_id`=?,`activity_type_id`=?,`article_id`=?,`image_path`=?,`activity_content`=?,`quick_tips`=?,`theme`=?,`difficulty_level`=?,`author`=?,`issue_no`=?,`audio_support`=?, `activity_html`=?,`activity_result`=?,`act_year`=? WHERE activity_id=? ");
			$stmt->bind_param("ssssssssssssssss", $param_article_title, $param_mag_id,  $param_activity_type_id,  $param_essay_type_id, $param_article_image, $param_article_content, $param_quick_tips, $param_theme,  $param_difficulty_level,  $param_author, $param_issue_no, $param_audio_support, $param_activity_html, $param_activity_result,$act_year, $param_aID);
			$param_article_title = $_POST['title'];
			$param_activity_type_id = $_POST['activity_type_id'];
			$param_article_published_date = $todaysDate;
			$param_mag_id = $mag_id_all;
			$param_article_status = $active;
			$param_essay_type_id = $article_id_new;
			$param_article_path = '';
			 $param_activity_html =$activity_html;
			 $param_activity_result=$activity_result;
			$param_issue_no = $_POST['issue_no'];
			$param_quick_tips = 'test';
			...
			if ($stmt->execute()) {
				...
				if ($_FILES['my_file']['name'] != '') {
					...
					$stmt1 = $mysqli->prepare("delete FROM edu_activity_audio where activity_id=?");
