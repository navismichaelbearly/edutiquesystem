...
if (isset($_POST['title']) || isset($_POST['activity_type_id']) || isset($_POST['file']) || isset($_POST['content']) || isset($_POST['article_id'])  || isset($_POST['audio_support']) || isset($_POST['activity_html']) || isset($_POST['activity_result'])) {
	...
	$newDate = date("Y-m-d", strtotime($act_year));
	...
	if (!empty($title)) {
		...
		if ($uploadStatus == 1) {
			...
			(activity_title, mag_id, activity_type_id, activity_published_date, activity_status, article_id, activity_path, image_path, activity_content, activity_style, audio_path, quick_tips, activity_html, activity_result, theme,  difficulty_level,  author, issue_no,audio_support,act_year)
			 VALUES ('" . $_POST['title'] . "','" . $mag_id_all . "','" . $activity_type_id . "','" . $todaysDate . "','" . $active . "','" . $article_id_new . "','" . '' . "','" . $param_article_image . "','" . $content . "','" . '' . "','" . '' . "','" . $quick_tips . "','" . $activity_html . "','" . $activity_result . "','" . $_POST['theme'] . "','" . $_POST['difficulty_level'] . "','" . $_POST['author'] . "','" . $_POST['issue_no'] . "','" . $_POST['audio_support'] . "','" . $_POST['act_year'] . "')");
